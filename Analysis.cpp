...
bool Analysis::Initialize(const MA5::Configuration& cfg, const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddHisto("M_b_dijet", 100, 0.0, 100.0); /*Name, bins, x_initial, x_final*/
  plot_M_b_dijet = new TH1F("M_b_dijet", "M_b_dijet", 100, 0.0, 100.0);
...
bool Analysis::Execute(SampleFormat& sample, const EventFormat& event)
{
	...
	if (FinalStateJs.size() == 4){
		MergedFullyHadronic(FinalStateJs, FinalStateBs, dijet, b_dijet);
	}
	if (FinalStateJs.size() == 2){
		MergedSemiLeptonic(FinalStateJs, FinalStateBs, dijet, b_dijet);
	}
	
	Manager()->FillHisto("M_dijet", dijet.M());
	plot_M_dijet->Fill(dijet.M());
	
	Manager()->FillHisto("M_b_dijet", b_dijet.M());
	plot_M_b_dijet->Fill(b_dijet.M());
